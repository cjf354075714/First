...
4，重写概念
网络层关注的是如何将分组从源端沿着网络路径发送到目标端
在计算机通信中，可靠通信是谁来实现？是网络还是端系统？我觉得是端系统
网络层向上传输层提供两中服务：
虚拟路径服务和数据报服务，现在互联网采用的是数据报服务

5，网络层的主要协议
ARP 协议、RARP 协议、ICMP 协议、IGMP 协议 + IP 协议

6，IP 地址
IP 地址是32 位的 2 进制数，层次上是网络 IP 地址 + 主机 IP 地址
IP 地址也分协议，IPv4 和 IPv6，现阶段使用的是 IPv4
IP 地址的分类：
A 类地址 = 8 位主机地址 + 24 位主机地址
B 类地址 = 16 位主机地址 + 16 位主机地址
C 类地址 = 24 位主机地址 + 8 位主机地址
D 类地址 = 多播组播地址
E 类地址 = 不关心

特殊的地址：
127.0.0.1 表示本地换回地址
169.254.0.0 windows 自己给自己凑合用的 IP 地址